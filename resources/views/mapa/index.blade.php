...
/* ── Tooltip ── */
.map-tooltip{position:absolute;background:#fff;border-radius:14px;padding:0;box-shadow:var(--shadow-lg);border:1px solid rgba(55,96,115,.15);min-width:220px;max-width:280px;pointer-events:auto;opacity:0;transform:translateY(8px) scale(.97);transition:opacity .25s ease,transform .25s var(--ease);z-index:200;display:none;}
...
<div class="campus-main">
    ...
    <div class="map-wrapper" id="mapWrapper">
        <svg class="campus-svg" viewBox="0 0 900 560" xmlns="http://www.w3.org/2000/svg">
            ...
            {{-- Área de Manufactura --}}
            {{-- Área de Manufactura --}}
            <g class="area-static" data-area="manufactura" transform="translate(710,90)">
                <rect width="140" height="70" rx="6" fill="#dde8ef" stroke="#376073" stroke-width="1.5" filter="url(#areaShadow)"/>
                <text x="70" y="30" text-anchor="middle" font-family="Inter,sans-serif" font-size="10" font-weight="700" fill="#376073">Manufactura</text>
                <text x="70" y="48" text-anchor="middle" font-family="Inter,sans-serif" font-size="8.5" font-weight="500" fill="#5a7a8a">Galería</text>
            </g>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const DATA = {
        ...
        b: {
            ...
            body:`<strong>Planta baja:</strong> EB1–EB9, Biblioteca, Lobby<br>
                  ...
                  <strong>Estado:</strong> Ver disponibilidad en tiempo real`,
            btn:{ text:'Ver disponibilidad →', href:'{{ route("aulas.edificioB") }}' }
        },
        c: {
            icon:'🔬',
            title:'Edificio C',
            sub:'Laboratorios y aulas',
            body:`<strong>Planta baja:</strong> Simulación, Automatización, Auditorio, Lobby<br>
                <strong>Planta alta:</strong> Lab. Tiempos y Movimientos, Lab. Metrología<br>
                <strong>Estado:</strong> Ver disponibilidad en tiempo real`,
            btn:{ text:'Ver disponibilidad →', href:'{{ route("aulas.edificioC") }}' }
        },
        'cancha-futbol': {
        ...
        sub: 'Talleres y laboratorios',
        body: 'Salones P1, P2 y P3 para prácticas de taller y laboratorio.',
        btn: { text: 'Ver disponibilidad', href: "{{ route('aulas.salonesPracticas') }}" }
    },
        manufactura: {
        icon: '⚙️',
        title: 'Manufactura',
        sub: 'Zona del Edificio C',
        body: 'Área de referencia para manufactura.<br>Disponible para consultar ubicación y galería.',
        btn: { text: 'Ver disponibilidad', href: "{{ route('aulas.manufactura') }}" }
    }
    };
    ...
    // ← Sin click directo, el usuario elige desde el tooltip
    g.addEventListener('click', () => showTooltip(key, g)); // solo mantiene el tooltip visible
});
        ...
        fotos.forEach((f, i) => {
            ...
            const adminBtns = esAdmin
                ? `<div class="gallery-card-actions">
                       <button class="btn-del-photo" data-id="${f.id}" onclick="deletePhoto(${f.id})">🗑 Eliminar</button>
                   </div>`
                : '';
            ...
            // Abrir lightbox al click en la imagen
            card.querySelector('.gallery-card-img').addEventListener('click', () => {
                openLightbox(imgUrl, f.titulo, f.descripcion);
            });
