...
@section('title','Edificio A - Ubicatec')
...
/* Botón de acción debajo del panel */
...
<div class="u-main">
    ...
    <div class="plants-grid">
        ...
        {{-- PLANTA BAJA --}}
        <div class="plant-block" id="planta-baja-a">
            <div class="plant-title">
                <svg viewBox="0 0 24 24"><rect x="3" y="9" width="18" height="13" rx="2"/><path d="M3 9l9-5 9 5"/></svg>
                Planta Baja
            </div>
            <div class="aula-row">
                <div class="aula-box" data-aula="EA01"><span class="aula-icon">🚪</span>EA1</div>
                <div class="aula-box" data-aula="EA02"><span class="aula-icon">🚪</span>EA2</div>
                <div class="label-box label-box-galeria" data-area="salida-canchas" title="Ver galería">
                    Salida a Canchas 🖼️
                </div>
                <div class="aula-box" data-aula="Of-BAJA"><span class="aula-icon">🏢</span>Of</div>
                <div class="aula-box" data-aula="CC1"><span class="aula-icon">💻</span>CC1</div>
            </div>
            <div class="panel-info" id="panel-baja">
                <div class="panel-placeholder">
                    <svg width="34" height="34" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><rect x="3" y="3" width="18" height="18" rx="3"/><path d="M3 9h18M9 21V9"/></svg>
                    <span>Selecciona un aula</span>
                </div>
            </div>
            <div id="accion-baja" style="display:none;"></div>
            <div class="aula-row">
                <div class="aula-box" data-aula="EA03"><span class="aula-icon">🚪</span>EA3</div>
                <div class="aula-box" data-aula="EA04"><span class="aula-icon">🚪</span>EA4</div>
                <div class="label-box label-box-galeria" data-area="escaleras-2da" title="Ver galería">
                    Escaleras 2da planta 🖼️
                </div>
                <div class="aula-box" data-aula="Banios-BAJA" data-tipo="banio"><span class="aula-icon">🚻</span>Baños</div>
                <div class="aula-box" data-aula="EALQ"><span class="aula-icon">🧪</span>Lab Quím.</div>
            </div>
        </div>

        {{-- PLANTA ALTA --}}
        <div class="plant-block" id="planta-alta-a">
            <div class="plant-title">
                <svg viewBox="0 0 24 24"><rect x="3" y="9" width="18" height="13" rx="2"/><path d="M3 9l9-5 9 5"/><line x1="12" y1="4" x2="12" y2="2"/></svg>
                Planta Alta
            </div>
            <div class="aula-row">
                <div class="label-box vertical label-box-galeria" data-area="oficinas" title="Ver galería">
                    Oficinas 🖼️
                </div>
                <div class="aula-box" data-aula="Of-ALTA"><span class="aula-icon">🏢</span>Of</div>
                <div class="aula-box" data-aula="A9"><span class="aula-icon">🚪</span>A9</div>
                <div class="aula-box" data-aula="A10"><span class="aula-icon">🚪</span>A10</div>
                <div class="label-box label-box-galeria" data-area="area-medica" title="Ver galería">
                    Área Médica 🖼️
                </div>
            </div>
            <div class="panel-info" id="panel-alta">
                <div class="panel-placeholder">
                    <svg width="34" height="34" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><rect x="3" y="3" width="18" height="18" rx="3"/><path d="M3 9h18M9 21V9"/></svg>
                    <span>Selecciona un aula</span>
                </div>
            ...
            <div class="aula-row">
                <div class="label-box label-box-galeria" data-area="escaleras-1ra" title="Ver galería">
                    Escaleras 1ra planta 🖼️
                </div>
                <div class="aula-box" data-aula="Banios-ALTA" data-tipo="banio"><span class="aula-icon">🚻</span>Baños</div>
                <div class="aula-box" data-aula="A5"><span class="aula-icon">🚪</span>A5</div>
                <div class="aula-box" data-aula="A7"><span class="aula-icon">🚪</span>A7</div>
            </div>
        </div>
...
<footer class="u-footer">
    © 2025 Ubicatec — Edificio A &nbsp;·&nbsp; {{ $dia }} {{ substr($hora,0,5) }}
</footer>
</div>
...
<div class="modal-overlay" id="modalApartar">
    <div class="modal-box">
        ...
        <form method="POST" action="{{ route('aulas.apartar') }}" id="formApartar">
            ...
            {{-- Slots: se muestran siempre para aulas normales --}}
            ...
            {{-- Override de hora: solo admin/jefe --}}
            {{-- Nota límite docente --}}
            <div id="nota-limite-docente" style="display:none;background:#e8fdf2;border:1px solid #27ae60;border-radius:8px;padding:7px 12px;font-size:.72rem;color:#27ae60;margin-bottom:4px;">
            ...
            {{-- Datos: aparece al seleccionar slot --}}
            <div id="seccion-datos-reserva" style="display:none;">
                ...
                <div id="campos-aula">
                    ...
                    <div id="resumen-horas" style="display:none;margin-top:8px;padding:7px 10px;background:#e8f4fd;border-radius:8px;font-size:.73rem;color:var(--teal);">
...
// DESPUÉS — usa hora del navegador (bien)
...
const ES_ESTUDIANTE = ROL === 'estudiante';                    // solo ver
const JORNADA_INI = 420;   // 07:00
const JORNADA_FIN = 1320;  // 22:00

let horaMin = HORA_REAL_MIN;
let aulaActiva = null;   // data-aula original (ej: "EA01")
let plantaActiva = null; // 'baja' | 'alta'
...
function normAula(raw){
    ...
    if(a==='EA01'||a==='EA1') return 'E101';
    if(a==='EA02'||a==='EA2') return 'E102';
    if(a==='EA03'||a==='EA3') return 'E103';
    if(a==='EA04'||a==='EA4') return 'E104';
    if(a==='CC1') return 'EAC1';
    const mA = a.match(/^A(\d{1,2})$/);   if(mA) return 'E1'+mA[1].padStart(2,'0');
    const mEA= a.match(/^EA(\d{1,2})$/);  if(mEA) return 'E1'+mEA[1].padStart(2,'0');
    return a;
}

// Detecta baño por el data-aula original (antes de normAula)
function esBanio(raw){ const a=norm(raw); return a.includes('BANIO')||a.includes('BAÑO'); }

/* ── Consultas de estado (usan horaMin del slider) ── */
...
/* ── Calcula slots del día para un aula ── */
...
/* ── HTML del panel de info (sin botón) ── */
...
<div class="info-row">
    ...
    // Próximos slots para no-estudiantes
    let slotsHTML = '';
    ...
    // Estudiantes: próximas disponibilidades
    let estudHTML = '';
    if(ES_ESTUDIANTE){
...
/* ── HTML del botón de acción (va en #accion-baja / #accion-alta) ── */
function accionHTML(raw){
    ...
    // Estudiantes: nunca botón
    if(ES_ESTUDIANTE) return '';

    // ── BAÑOS ──
    if(banio){
        if(!ES_ADMIN) return ''; // docentes no tocan baños
        if(bloq){
            return `<button class="btn-apartar" onclick="abrirModal('${raw}',true)">🔧 Modificar estado del baño</button>`;
        }
    ...
    // ── AULAS NORMALES ──
    if(ES_ADMIN){
        // Admin/Jefe: siempre puede, sin importar si hay clase
        return `<button class="btn-apartar" onclick="abrirModal('${raw}',false)">📌 Apartar / Reservar aula</button>`;
    }

    if(ES_DOCENTE){
        // Docente libre ahora: solo botón "Apartar aula"
        if(!clase && !bloq){
            return `<button class="btn-apartar" onclick="abrirModal('${raw}',false)">📌 Apartar aula</button>`;
        }
        // Docente con aula ocupada/bloqueada: aviso + botón para otro horario si hay slots libres
        const btnOtroHorario = hayLibre
            ? `<button class="btn-apartar" style="margin-top:6px;background:linear-gradient(135deg,#27ae60,#2ecc71);" onclick="abrirModal('${raw}',false)">📅 Apartar en otro horario</button>`
            : '';
        return `<div class="aviso-no-disponible">⛔ Ocupada en este horario</div>${btnOtroHorario}`;
    }
...
function seleccionar(raw, planta){
    ...
    otrAcc.style.display  = 'none';
}

document.querySelectorAll('#planta-baja-a .aula-box').forEach(el=>{
    el.addEventListener('click', ()=>seleccionar(el.dataset.aula,'baja'));
});
document.querySelectorAll('#planta-alta-a .aula-box').forEach(el=>{
...
function actualizarSlider(min){
    ...
    sliderLbl.textContent = lbl;
    horaDisplay.textContent = lbl;
...
const ALIAS_CARRERA = {
    // Contaduría
    'cont': 'c', 'conta': 'c', 'contad': 'c', 'contaduria': 'c', 'contaduría': 'c',
    // Sistemas
    'sist': 's', 'sistema': 's', 'sistemas': 's', 'computac': 's', 'comp': 's',
    // Industrial
    'indu': 'i', 'indus': 'i', 'industrial': 'i',
    // Electromecanica
    'elec': 'e', 'electro': 'e', 'mecan': 'e', 'electromecanica': 'e', 'electromecánica': 'e',
    // Gestión
    'gest': 'g', 'gesti': 'g', 'gestion': 'g', 'gestión': 'g', 'empresa': 'g', 'empresarial': 'g',
    // Turismo
    'tur': 't', 'turism': 't', 'turismo': 't',
    // Ambiental
    'ambi': 'a', 'ambient': 'a', 'ambiental': 'a',
};
...
function resolverCarrera(q) {
    if (!q) return null;
    // Busca si q está contenido en el alias O el alias está contenido en q
    for (const [alias, letra] of Object.entries(ALIAS_CARRERA)) {
        if (alias.includes(q) || q.includes(alias)) return letra;
    }
...
/* ── Buscador ── */
const PANEL_PRINCIPAL = 'panel-baja';
const getPlanta = el => el?.closest('#planta-baja-a') ? 'baja' : 'alta';
...
document.getElementById('buscador').addEventListener('input', function () {
    ...
    const pareceAula = /^(eb|ea|e1|e2|cc|ec|of|lab|ban|ealq|a\d)/i.test(q) || /^\d/.test(q);
    ...
    if (pareceAula || !pareceCarrera) {
        ...
        const exacta = todasAulas.find(e => {
            ...
            const sinCero = cod.replace(/^ea0/, 'ea');
        ...
        if (parciales.length > 1) {
            ...
            const filas = parciales.map(e => {
                ...
                return `<div class="info-row" style="cursor:pointer;padding:5px 4px;border-radius:7px;transition:background .15s;"
                    ...
                    <span class="info-val" style="color:var(--muted);font-size:.75rem;">${planta === 'baja' ? 'Planta Baja' : 'Planta Alta'}</span>
                </div>`;
            }).join('');
...
function abrirModal(raw, esBloqueo){
    ...
    if(esBloqueo){
        // Baño: ocultar slots, mostrar datos directo
        ...
        // Poner hora completa del día para el bloqueo
        ...
        document.getElementById('modal-nota').textContent       = '⏳ El apartado expira al inicio de la siguiente semana';
...
document.getElementById('modal-nota').textContent = ES_DOCENTE
    ...
    : '⏳ El apartado expira al inicio de la siguiente semana';        document.getElementById('campos-aula').style.display    = 'block';
...
function elegirSlot(hi, hf, override, btn){
    ...
    document.getElementById('seccion-datos-reserva').style.display = 'block';
    const dur=hf-hi, h=Math.floor(dur/60), m=dur%60;
    document.getElementById('duracion-texto').textContent = h>0?`${h}h ${m}m`:`${m} min`;
    document.getElementById('resumen-horas').style.display = 'block';
    const dia  = document.getElementById('modal-dia').value;
    const prev = document.getElementById('modal-preview');
    prev.style.background = override?'#fff3cd':'#e8f4fd';
    prev.style.color       = override?'#b7770d':'var(--teal)';
    prev.innerHTML = `📋 <strong>${normAula(aulaActiva)}</strong> — ${dia} · <strong>${minToH(hi)}</strong> a <strong>${minToH(hf)}</strong>
        &nbsp;|&nbsp; ⏱️ ${h>0?h+'h ':''} ${m}m
        ${override?'&nbsp;<span style="color:#b7770d;font-weight:700">⚠️ Override</span>':''}`;
...
function aplicarHoraManual(){
    ...
    if(!hi||!hf){ mostrarError('Completa hora inicio y fin.'); return; }
    const hiM=hToMin(hi), hfM=hToMin(hf);
    if(hfM<=hiM){ mostrarError('La hora fin debe ser mayor.'); return; }
    const fake=document.createElement('button'); fake.className='modal-slot';
    elegirSlot(hiM,hfM,true,fake);
}
...
document.getElementById('formApartar').addEventListener('submit', function(e){
    ...
    if(!hi||!hf){ e.preventDefault(); mostrarError('⚠️ Selecciona un horario.'); return; }
    const hiM=hToMin(hi), hfM=hToMin(hf);
    if(hfM<=hiM){ e.preventDefault(); mostrarError('⚠️ Hora fin debe ser mayor.'); return; }
    if(!ES_ADMIN){
        const bd  = normAula(aulaActiva);
        const dia = document.getElementById('modal-dia').value;
        const cc  = HORARIOS.some(h=>normAula(h.aula)===bd&&norm(h.dia)===norm(dia)&&hToMin(h.hora_inicio)<hfM&&hToMin(h.hora_fin)>hiM);
        if(cc){ e.preventDefault(); mostrarError('❌ Conflicto con clase existente.'); return; }
        const ca  = APARTADOS.some(ap=>normAula(ap.aula)===bd&&norm(ap.dia)===norm(dia)&&hToMin(ap.hora_inicio)<hfM&&hToMin(ap.hora_fin)>hiM);
        if(ca){ e.preventDefault(); mostrarError('❌ Ese horario ya está apartado.'); return; }
    }
    // Docente: límite de 2 horas
    if(ES_DOCENTE && (hfM - hiM) > 120){
        e.preventDefault(); mostrarError('⚠️ Los docentes solo pueden reservar hasta 2 horas.'); return;
    }
});
